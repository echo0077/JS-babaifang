...
var checkReg = {
    ...
    nickname: function (str) {//昵称只能由汉字和字母组成
        var reg = /^[A-Za-z0-9\u4e00-\u9fa5]+$/;
        return reg.test(str);
    },
    username: function (str) {
        var reg = /^[a-zA-Z][\w\-]{5,19}$/;
        return reg.test(str);//字母开头，允许5-19字节
    },
    identity: function (str) {
        var reg = /^([0-9]){7,18}(x|X)?$/;
        return reg.test(str);//数字、字母x结尾
    },
    birthday: function (str) {
        var reg = /^(19|20)\d{2}(-|.)(1[0-2]|0?[1-9])$/;
        return reg.test(str);//日期在1900-0到2099-12之间
    },
    password: function (str) {
        var reg = /^\S[a-zA-Z]\w{5,17}$/;
        return reg.test(str);
        //以字母开头(前两位)，长度在6~18之间，只能包含字母、数字和下划线,不能有空格
    },
}
